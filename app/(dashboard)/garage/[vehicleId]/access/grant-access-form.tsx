...
export function GrantAccessForm({ vehicleId }: { vehicleId: string }) {
  ...
  return (
    <form action={action} className="space-y-3">
      ...
      {isSearching && <p className="text-xs text-slate-400">Mencari...</p>}

      {results.length > 0 && !selected && (
        <div className="space-y-1.5">
          {results.map((w) => (
            <button key={w.id} type="button" onClick={() => { setSelected(w); setQuery(w.name); setResults([]); }}
              className="w-full text-left rounded-2xl border border-slate-200 bg-white px-3 py-2 text-sm">
      ...
      {selected && (
        <div className="bg-green-50 border border-green-200 rounded-2xl px-3 py-2 text-sm text-green-800">
          Terpilih: {selected.name}
        </div>
      )}

      <button type="submit" disabled={!selected || isPending}
        className="w-full rounded-2xl bg-brand-600 text-white py-2.5 font-medium disabled:opacity-50">
