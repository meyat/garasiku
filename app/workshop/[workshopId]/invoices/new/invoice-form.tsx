...
export function InvoiceForm({ workshopId }: { workshopId: string }) {
  ...
  return (
    <form action={action} className="space-y-4">
      <div>
        <label className="text-sm font-bold text-slate-700">Nomor Invoice</label>
        <input name="invoiceNumber" required placeholder="INV-001"
          className="mt-1 w-full rounded-2xl border border-slate-200 px-3 py-2" />
      </div>
      <div className="grid grid-cols-2 gap-2">
        <input name="customerName" placeholder="Nama pelanggan" className="rounded-2xl border border-slate-200 px-3 py-2 text-sm" />
        <input name="customerPhone" placeholder="No. HP" className="rounded-2xl border border-slate-200 px-3 py-2 text-sm" />
      </div>

      <div>
        <p className="text-sm font-bold text-slate-700 mb-2">Item</p>
        <div className="space-y-2">
          {rows.map((row, i) => (
            <div key={i} className="flex gap-2 items-center">
              <input name="itemDescription" defaultValue={row.description} placeholder="Deskripsi" required
                className="flex-1 rounded-xl border border-slate-200 px-2 py-1.5 text-sm" />
              ...
              {rows.length > 1 && (
                <button type="button" onClick={() => setRows(rows.filter((_, idx) => idx !== i))} className="text-rose-500">
                  <Trash2 size={16} />
                </button>
              )}
            </div>
        ...
        <button type="button" onClick={() => setRows([...rows, { description: "", quantity: 1, unitPrice: 0 }])}
          className="mt-2 text-xs text-brand-600 font-bold flex items-center gap-1">
          <Plus size={12} /> Tambah Item
        </button>
      </div>

      <div className="bg-slate-50 border border-slate-100 rounded-2xl p-4 flex justify-between">
        <span className="text-sm font-bold text-slate-700">Total</span>
        <span className="text-lg font-bold text-slate-900">Rp{Math.round(total).toLocaleString("id-ID")}</span>
      </div>

      <div>
        <label className="text-sm font-bold text-slate-700">Catatan (opsional)</label>
        <textarea name="notes" rows={2} className="mt-1 w-full rounded-2xl border border-slate-200 px-3 py-2 text-sm" />
      </div>
